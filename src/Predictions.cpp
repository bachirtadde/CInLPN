...
arma::mat predi(int K, int nD, arma::mat matrixP, int m_i, arma::vec tau, arma::vec tau_i, arma::mat Ytild_i,
  ...
  mat pred = zeros(m_i,(2*K)); // matrix of all predictions (marginal and subject-specific)
  vec vect = ones(nD);
  ...
  pred_MYtildFull = mat_Nu_i*matrixP.t(); // full marginal prediction in transformed scale
  
  vec predMyi = zeros(m_i*K); //  Marginal Prediction in real scale
  ...
  for(int nr=0; nr < MCnr; nr++){
    ...
    mat mat_yMn1 = zeros(m_i,K); // yn1 = H^-1(ytildM_i)
    ...
    for(int k=0; k<K; k++){
      if(if_link[k] == 1){
        for(int j=0; j< m_i;j++){
          // for marginal pred
          int lM = 0;
          while((mat_ytildM_i(j,k) > GrilleYtild(lM,k)) & (lM< (m_i-1))){
            lM += 1;
          }
  ...
  for(int k=0; k<K; k++){
    pred.col(ii) = pred_MYtildFull.col(k);
    pred.col(ii+1) = pred_MYFull.col(k);
